...
function lembrar() {
    ...
    div.innerHTML = '<input type="submit" name="conf" value="' + tipoPreenchimento + '" id="botaoVermelho" onclick="lembrar()">\
				<input type="button" value="Voltar" onclick="voltar()">';
...
function botaoInscrever(nomeValidado, CPFValidado, senhaValidada, confirmarSenhaValidada, CAPTCHAValidado, eMailValidado) {
    ...
    if (nomeValidado, CPFValidado, senhaValidada, confirmarSenhaValidada, CAPTCHAValidado, eMailValidado) {
        div.innerHTML = '<input type="submit" name="conf" value="' + tipoPreenchimento + '">\
				<input type="button" value="Voltar" onclick="voltar()">';
        ...
        div.innerHTML = '\
			<input type="submit" name="conf" value="' + tipoPreenchimento + '" id="botaoVermelho" onclick="lembrar()">\
			<input type="button" value="Voltar" onclick="voltar()">';
        $('#alertaPreenchimento').hide();
    }
}
...
function selecionarParticipacao() {
    $('.areaOrganizadores').css({textAlign: "left"});
    gerenciar();
}
...
function posicionarMenu () {
        var limites = {superior: 0, inferior: 0}
                , menuContainer = $('.menu')
                , menu = $('.scrollable')
                , content = $('.content')
                , wnd = $(window)
                , deslocamento = 210
                , rodape = {offset: 0, topo: 0, base: 0};

        wnd.on('scroll', function(event) {
            var offset = menuContainer.offset();
            limites.superior = offset.top;
            limites.inferior = offset.top + menuContainer.height() - menu.height();
            rodape.offset = content.offset();
            rodape.topo = rodape.offset.top;
            rodape.base = rodape.offset.top + menuContainer.height() - menu.height();
            ...
            if (st < deslocamento && st > wnd.scrollTop() - rodape.topo) {
                menu.css({bottom: st - deslocamento});
                stN = st;
            }
    ...
    if (hash !== "") {
        ...
        if (node + 745 < heightSize && $(document).scrollTop() == 0) {
            ...
            if (node + 745 < heightSize) {
                $('html, body').animate({scrollTop: node + 60});
            }
        }
    }
}
...
// Index - Menu
    ...
    $('a.ancora').click(function() {
        ...
        $("html:not(:animated),body:not(:animated)").animate({scrollTop: destination - 200}, 0);
        return false;
    });
...
// Contato - Formulário - Campo focado
    $("input").focus(function() {
        ...
        switch ($(this).attr('id')) {
            case "nomeCompleto":
                nomeValidado = true;
                botaoEnviar(nomeValidado, eMailValidado, assuntoValidado, mensagemValidada);
                break;
            case "usuario":
            case "CPF":
                CPFValidado = true;
        ...
        if (teste)
            botaoInscrever(nomeValidado, CPFValidado, senhaValidada, confirmarSenhaValidada, CAPTCHAValidado, eMailValidado);
    });
...
// Contato - Formulário - Campo desfocado
    $("input").blur(function() {
        ...
        switch (idCampo) {
            case "nomeCompleto":
                ...
                div = document.getElementById("validarNome");
                break;
            case "usuario":
            case "CPF":
                valorCampo = "CPF";
                div = document.getElementById("validarCPF");
                break;
            case "senha":
                valorCampo = "Senha";
                div = document.getElementById("validarSenha");
                break;
            case "confirmarSenha":
                valorCampo = "Confirmar senha";
                div = document.getElementById("validarConfirmarSenha");
                break;
            case "captcha_code":
                valorCampo = "CAPTCHA";
                div = document.getElementById("validarCAPTCHA");
                break;
            case "eMail":
                valorCampo = "e-mail";
                div = document.getElementById("validarEMail");
                break;
            default:
                teste = false;
        }

        if (document.getElementById(idCampo).value == "" && teste) { // Falsea um campos simples
            $(this)
                    .parent()
                    .addClass("focoAtual")
                    .children("div")
                    .toggle();
            div.innerHTML = "<p id='validarFormulario'>Por favor, preencha o campo \"" + valorCampo + "\"</p>";
        ...
        else if (idCampo == "CPF" || idCampo == "usuario") {
            var CPF;
            if (idCampo == "CPF")
                CPF = document.getElementById("CPF").value;
            else
                CPF = document.getElementById("usuario").value;
        ...
        else { // Corrobora um campo simples
            $(this)
                    .parent()
                    .removeClass("focoAtual")
                    .children("div")
	...
	</div><div class="Assunto"></div><div class="Mensagem"></div>').appendTo("div.formulario");
    ...
    $("select").blur(function() {
        div = document.getElementById("validarAssunto");

        var texto
        texto = "<br>O número de opções do select: " + document.formulario.assunto.length
        var indice = document.formulario.assunto.selectedIndex
        texto += "<br>Indice da opção escolhida: " + indice
        var valor = document.formulario.assunto.options[indice].value
        texto += "<br>Valor da opção escolhida: " + valor
        var textoEscolhido = document.formulario.assunto.options[indice].text
        texto += "<br>Texto da opção escolhida: " + textoEscolhido
    ...
    $("textarea").blur(function() {
        if ($(this).attr('id') === "mensagem") {
            div = document.getElementById("validarMensagem");
    ...
    $('<div id="validarNome"></div><div id="validarEMail"></div><div id="validarAssunto"></div><div id="validarMensagem"></div>').appendTo('<div id="validarFormulario"></div>');
});
